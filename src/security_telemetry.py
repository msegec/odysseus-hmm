...
This module provides a central ``record_security_event(...)`` API plus an
append-only ledger whose every row carries full
provenance (raw signals, classifier rule + rule version, confidence, code
refs, local timestamp) so any claim is replayable.

Storage choice: an append-only JSONL file under
``data/security_telemetry/``, not a DB table. Why:

* ``data/`` and ``*.db`` are gitignored, so the ledger is *guaranteed*
  local-only and never committed to git, by construction rather than by policy.
